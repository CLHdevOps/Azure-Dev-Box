...
function Invoke-7zipDownload() {
	...
	if (Test-Path -Path $7zipTool -PathType Leaf) {	
		write-host 'AIB Customization: 7zip location: $7zipTool'
		return $7zipTool 
	}
...
write-host 'AIB Customization: Downloading 7zip'
$7zip = Invoke-7zipDownload

write-host 'AIB Customization: Downloading WuT COM Redirector'
...
write-host 'AIB Customization: Extracting WuT COM Redirector'
$process = Start-Process $7zip -ArgumentList "x $archive -o$extract" -NoNewWindow -Wait -PassThru
if ($process.ExitCode -ne 0) { exit $process.ExitCode }

write-host 'AIB Customization: Extracting WuT COM Redirector (Installer)'
...
write-host 'AIB Customization: Installing WuT COM Redirector'
