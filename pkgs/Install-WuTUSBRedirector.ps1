...
write-host 'AIB Customization: Downloading WuT USB Redirector'
...
write-host 'AIB Customization: Extracting WuT USB Redirector'
$process = Start-Process $7zip -ArgumentList "x $archive -o$extract" -NoNewWindow -Wait -PassThru
if ($process.ExitCode -ne 0) { exit $process.ExitCode }

write-host 'AIB Customization: Extracting WuT USB Redirector (Installer)'
...
write-host 'AIB Customization: Installing WuT USB Redirector'
